# Cutline Creator Pack SDK 1.0

## 목적

Creator Pack은 코드를 실행하지 않고 Cutline의 제작 자산을 묶어 배포하는 선언형 확장 형식이다. Pack은 네트워크·파일시스템·임의 코드 실행 권한을 가질 수 없으며, 설치 전에 호환성·크기·개수·SHA-256 무결성과 선택적 Ed25519 제작자 서명을 검사한다.

- Pack 스키마: `cutline-creator-pack-v2`
- 현재 Pack API: `1.0.0`
- JSON Schema: [`../schemas/cutline-creator-pack-v2.schema.json`](../schemas/cutline-creator-pack-v2.schema.json)
- Catalog Schema: [`../schemas/cutline-creator-catalog-v1.schema.json`](../schemas/cutline-creator-catalog-v1.schema.json)
- 최대 직렬화 크기: 2MB
- 지원 자산: 모션, 속도, 오디오 팀, 타이틀 스타일, 출력, 전환 프리셋

## 제작과 배포 흐름

1. Cutline에서 사용할 템플릿과 프리셋을 만든다.
...
Ed25519 서명 검증은 “이 파일이 포함된 공개키로 서명됐다”는 사실을 확인한다. Cutline은 사용자가 신뢰·차단한 공개키와 SHA-256 키 지문을 로컬에 저장하고 설치된 Pack에 지문을 고정한다. 최초 공개키가 실제 제작자의 것인지 자동으로 보장하는 마켓플레이스 인증서·외부 신뢰 체인은 별도 상용 인프라로 남아 있다.
...
| 형식 | 권장 용도 | 보존 수준 |
| --- | --- | --- |
| OTIO | Cutline 및 메타데이터 친화 파이프라인 | 마커·트랙 설정·변형·전환 등 Cutline 확장 메타데이터 보존 |
...
교환 파일은 프로젝트 자체의 완전한 백업 형식이 아니다. 고급 효과, 생성 자산, 일부 자동화는 원본 Cutline 프로젝트와 함께 보관해야 한다.

## 카탈로그·업데이트 배포 계약

운영 카탈로그는 `cutline-creator-catalog-v1` 형식이며 최대 1MB, Pack·회수 항목 각각 최대 1,000개다. 앱은 카탈로그 서명을 빌드에 고정된 Ed25519 공개키와 `keyId`로 확인한다.

```text
VITE_CUTLINE_CREATOR_CATALOG_URL=https://packs.example.com/catalog.json
VITE_CUTLINE_CREATOR_CATALOG_PUBLIC_KEY=<Ed25519 raw public key, Base64>
VITE_CUTLINE_CREATOR_CATALOG_KEY_ID=cutline-pack-catalog-2026
```
