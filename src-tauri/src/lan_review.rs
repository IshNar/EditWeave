...
fn ensure_server() -> Result<&'static ReviewServer, String> {
    ...
    std::thread::Builder::new().name("cutline-lan-review".into()).spawn(move || {
        for stream in listener.incoming() {
            let Ok(stream) = stream else { continue; };
            let sessions = worker_sessions.clone();
            let _ = std::thread::Builder::new().name("cutline-review-client".into()).spawn(move || { let _ = handle_request(stream, sessions); });
        }
...
mod tests {
    ...
    fn lan_review_serves_video_and_synchronizes_comments() {
        ...
        let token = format!("cutlinereviewtoken{nonce}");
        let path = std::env::temp_dir().join(format!("cutline-lan-review-{nonce}.mp4"));
        ...
        assert!(page.contains("CUTLINE REVIEW"));
