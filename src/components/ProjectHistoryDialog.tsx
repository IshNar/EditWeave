...
import type { CutlineProjectDocument, ProjectMergeConflictRecord, ProjectMergeSession } from '../editor/types'
...
interface ProjectHistoryDialogProps {
  open: boolean
  snapshots: CutlineProjectDocument[]
  currentProject: CutlineProjectDocument
  onClose: () => void
  onRestore: (snapshot: CutlineProjectDocument) => void
  onBranch: (snapshot: CutlineProjectDocument, sequenceId: string) => void
  onMerge: (base: CutlineProjectDocument, incoming: File) => void
...
export function ProjectHistoryDialog({ open, snapshots, currentProject, onClose, onRestore, onBranch, onMerge, mergeSessions, onResolveConflict, onLocateConflict }: ProjectHistoryDialogProps) {
  const [selected, setSelected] = useState<CutlineProjectDocument | undefined>()
