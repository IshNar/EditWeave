...
const STORAGE_KEY = 'editweave.effect-presets.v1'

export interface EffectPreset {
  id: string
  name: string
  version: 'editweave-effect-preset-v1'
  createdAt: string
...
export function serializeEffectPreset(preset: EffectPreset): string {
  return JSON.stringify({ format: 'editweave-effect-preset', version: 1, preset: clonePreset(preset) }, null, 2)
}
...
export function parseEffectPreset(raw: string): EffectPreset {
  ...
  if (!isEffectPreset(candidate)) throw new Error('지원되는 EditWeave 효과 프리셋이 아닙니다.')
...
function isEffectPreset(value: unknown): value is EffectPreset {
  ...
  return candidate.version === 'editweave-effect-preset-v1'
