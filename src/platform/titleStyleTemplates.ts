...
const STORAGE_KEY = 'cutline.title-style-templates.v1'

export interface TitleStyleTemplate {
  id: string
  name: string
  version: 'cutline-title-style-v1'
  createdAt: string
  style: CaptionStyle
}
...
export function createTitleStyleTemplate(name: string, style: CaptionStyle): TitleStyleTemplate {
  return { id: crypto.randomUUID(), name: name.trim() || '타이틀 스타일', version: 'cutline-title-style-v1', createdAt: new Date().toISOString(), style: structuredClone(style) }
}

export function serializeTitleStyleTemplate(template: TitleStyleTemplate): string {
  return JSON.stringify({ format: 'cutline-title-style', version: 1, template }, null, 2)
}
...
export function parseTitleStyleTemplate(raw: string): TitleStyleTemplate {
  ...
  if (!isTitleStyleTemplate(candidate)) throw new Error('지원되는 Cutline 타이틀 스타일이 아닙니다.')
...
function isTitleStyleTemplate(value: unknown): value is TitleStyleTemplate {
  ...
  return candidate.version === 'cutline-title-style-v1' && typeof candidate.id === 'string' && typeof candidate.name === 'string' && typeof candidate.createdAt === 'string' && Boolean(candidate.style && typeof candidate.style === 'object')
}
